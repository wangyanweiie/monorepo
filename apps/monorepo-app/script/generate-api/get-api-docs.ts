...
async function writeAPI() {
    try {
        // 将需要用到的后端swagger接口地址放进来
        const apiList = [
            'http://192.168.3.201:8002/v3/api-docs?group=%E7%B3%BB%E7%BB%9F%E6%9C%8D%E5%8A%A1',
        ];

        // 收集所有后端swagger返回的数据
        const res: unknowType = await Promise.all(
            apiList.map(async (items) => {
                const respon = await axios.get(items as string);
                if (!respon) {
                    return [];
                }
                return respon;
 ...
 */
export function arrayDeduplicate(arr: unknowType[], name: string) {
    const hash: unknowType = {};
    let hashList = JSON.parse(JSON.stringify(arr));
    hashList = hashList.reduce((total: unknowType, currentValue: unknowType) => {
