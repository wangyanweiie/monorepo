...
const handleCheckAllChange = (val: CheckboxValueType) => {
    ...
    columnList.value = checkAll.value
        ...
        : cloneDeep(props.modelValue).map((item: Column) => ({ ...item, checked: false }));
};
...
watch(
    ...
    newValue => {
        // 过滤出选中状态的表格列配置子项
        const newColumnList = newValue.filter((item: Column) => item.checked);
...
onBeforeMount(() => {
    ...
    columnList.value = cloneDeep(props.modelValue).map((item: Column) => ({
        ...item,
        checked: true,
    }));
});
